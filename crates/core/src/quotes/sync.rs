            ...
            .await
        {
            Ok(mut quotes) => {
                ...
                if quotes_count > 0 {
                    ...
                    // WC-40: record the attempt so an unfillable backfill window
                    // isn't re-scheduled every sync. (Upstream intentionally dropped
                    // the update_after_sync call here — no data means not "synced".)
                    self.record_backfill_attempt_if_needed(asset, plan).await;
                    AssetSyncResult {
                        asset_id,
                        quotes_added: 0,
