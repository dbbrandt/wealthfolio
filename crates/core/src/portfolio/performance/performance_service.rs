...
impl PerformanceService {
    ...
    async fn calculate_account_group_performance(
        ...
        // Use upstream's shared compute_account_performance for TWR/MWR calculation
        Self::compute_account_performance(&aggregated, None, start_date_opt, true).map(
            |mut metrics| {
                metrics.id = group_name.to_string();
                metrics
            },
        )
    }
