...
emits one SPLIT row for every account that HELD the security BEFORE the split
date and does NOT already have that split. Re-running after importing naturally
drops the now-existing rows.
...
def main():
    ...
    for a in assets:
        ...
        for sdate, num, den in ys:
            ...
            audit["splits"] += 1
            # accounts that HELD this asset before the split date
    ...
    print(f"post-split-only (skip)  : {audit['post_split_only']}", file=sys.stderr)
